 ...
 * File: account-info.page.js - Xu ly logic phia client (Frontend JavaScript).
 */
(function accountInfoPageModule(namespace) {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      if (!fullname) {
        namespace.toast.warning("Họ và tên không được để trống.");
        return;
      }

      if ((oldPassword && !newPassword) || (!oldPassword && newPassword)) {
        namespace.toast.warning("Vui lòng nhập đầy đủ Mật khẩu cũ và Mật khẩu mới.");
        return;
      }
      ...
      try {
        ...
        namespace.toast.success("Cập nhật thông tin thành công!");
