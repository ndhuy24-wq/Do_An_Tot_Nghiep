...
(function adminAccountsPageModule(namespace) {
  ...
  function updateCurrentSessionIfNeeded(updatedUser) {
    ...
    if (String(updatedUser.status || "").toUpperCase() === "LOCKED") {
      namespace.toast.warning("Bạn vừa khóa chính tài khoản admin hiện tại. Hệ thống sẽ đăng xuất.");
    ...
    if (!namespace.utils?.isAdminRole?.(updatedUser.role)) {
      namespace.toast.warning("Bạn vừa đổi tài khoản hiện tại khỏi quyền admin. Hệ thống sẽ quay về trang chủ.");
      namespace.session?.setCurrentUser?.(nextUser);
      window.location.href = "main.html";
    }
  }
    ...
    document.querySelectorAll(".btn-toggle-lock").forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          namespace.toast.error(error.message || "Không cập nhật được trạng thái tài khoản");
        }
      });
    });
  }
  ...
  async function handleSubmit(event) {
    ...
    if (!editingUserId) {
      namespace.toast.warning("Hãy bấm nút Sửa ở một tài khoản trước khi lưu.");
      return;
    }
    ...
    if (!fullname) {
      namespace.toast.warning("Họ tên không được để trống.");
      return;
    }
    ...
    try {
      ...
      renderTable();
      fillForm(updated);
      namespace.toast.success("Cập nhật tài khoản thành công!");
    } catch (error) {
      namespace.toast.error(error.message || "Lưu tài khoản thất bại");
    }
  }
